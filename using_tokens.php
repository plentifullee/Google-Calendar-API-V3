...
if(isset($feed->{'nextPageToken'})) {
	$cal_next_page_token = $feed->{'nextPageToken'};
	echo "<b>Next page token:</b> ".$cal_next_page_token."<br><br>";
	echo "<hr>";
	$path = "https://www.googleapis.com/calendar/v3/calendars/$email/events?maxResults=$maxResults&orderBy=startTime&singleEvents=true&timeMax=$timeMax&timeMin=$timeMin&pageToken=$cal_next_page_token&key=$api_key";
	$feed = json_decode(file_get_contents($path));
	//process results again using a page token
	parseJSON($feed);

} else {
	$cal_next_page_token = "No next page token!";
	echo "<b>Next page token:</b> ".$cal_next_page_token."<br><br>";
}
